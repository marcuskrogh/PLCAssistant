...
def _is_stock_board_needing_status_upgrade(text: str) -> bool:
    ...
    ``plcassistant_dashboard_version`` of 1–20 (SWD-137…222 start/cascade). Boards
    ...
    if "sensor.plcassistant_status" not in text:
        return True
    # Only refresh when an explicit older stock version is present (not 21+).
    if re.search(r"plcassistant_dashboard_version:\s*(?:[1-9]|1[0-9]|20)\b", text):
        return True
    return False
