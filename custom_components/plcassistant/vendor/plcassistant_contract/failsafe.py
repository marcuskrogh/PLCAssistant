...
def apply_input_policy(
    ...
    *,
    ...
    if binding.stale_after_s is not None and ctx.has_good:
        if ts is None or ctx.last_update_ts is None:
            # Stale window configured but no clock/timestamps — cannot evaluate
            # freshness; apply unavailable policy and mark not-stale explicitly.
            ctx.stale = False
            return _apply_policy(binding.unavailable_policy, binding, ctx)
        if (ts - ctx.last_update_ts) >= binding.stale_after_s:
            ctx.stale = True
