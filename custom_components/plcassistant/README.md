...
integration under Devices & services. After App Update, if Core has not yet
reloaded, Settings → System → Updates shows **Restart of Home Assistant
required** on the PLCAssistant update card (App **0.1.27+**). The integration
installs `/config/dashboards/plcassistant.yaml` (if missing) and adds
