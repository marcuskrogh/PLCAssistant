...
install_lovelace_dashboard() {
  ...
  # (SWD-135) or explicitly on dashboard_version 1–12 (SWD-137…166 Dynamics editor).
  ...
  if [ ! -f "${dst_dash}" ]; then
    ...
    && { ! grep -q 'sensor.plcassistant_status' "${dst_dash}" 2>/dev/null \
      || grep -qE 'plcassistant_dashboard_version:[[:space:]]*([1-9]|1[0-2])([^0-9]|$)' "${dst_dash}" 2>/dev/null; }; then
    cp -a "${src_dash}" "${dst_dash}" || true
