...
def test_app_version_0_1_53():
    ...
    assert '"0.1.66"' in (root / "manifest.json").read_text(encoding="utf-8")
    assert '"0.1.66"' in (dual / "manifest.json").read_text(encoding="utf-8")
    assert 'version: "0.1.66"' in Path("plc_assistant/config.yaml").read_text(encoding="utf-8")
    assert "BUILD_VERSION=0.1.66" in Path("plc_assistant/Dockerfile").read_text(encoding="utf-8")
