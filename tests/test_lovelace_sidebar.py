...
def test_app_version_locked_to_integration() -> None:
    ...
    assert 'version: "0.1.36"' in text
    docker = (ROOT / "plc_assistant" / "Dockerfile").read_text(encoding="utf-8")
    assert "BUILD_VERSION=0.1.36" in docker
...
def test_run_sh_refreshes_stock_missing_status_not_custom() -> None:
    ...
    # Explicit old versions only — refresh 1–15 stock boards to v16 (SWD-170).
    assert "plcassistant_dashboard_version:[[:space:]]*([1-9]|1[0-5])" in text
    ...
    assert "! grep -q 'plcassistant_dashboard_version: 16'" not in text
