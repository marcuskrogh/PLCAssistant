...
def test_app_version_locked_to_integration() -> None:
    ...
    assert 'version: "0.1.23"' in text
    docker = (ROOT / "plc_assistant" / "Dockerfile").read_text(encoding="utf-8")
    assert "BUILD_VERSION=0.1.23" in docker
...
def test_ensure_refreshes_stock_board_missing_status(tmp_path) -> None:
    ...
    assert "sensor.plcassistant_status" in text
    assert "sensor.plcassistant_mode" in text
    assert "plcassistant_dashboard_version: 11" in text


def test_ensure_refreshes_stock_board_old_dashboard_version(tmp_path) -> None:
    """SWD-144: stock boards on version 10 get unit-op authoring HMI help."""
    ...
    dest.parent.mkdir(parents=True)
    dest.write_text(
        "# plcassistant_dashboard_version: 10\n"
        "title: PLCAssistant\nviews:\n"
        "  - cards:\n"
        "      - type: entities\n"
        "        entities:\n"
    ...
    text = out.read_text(encoding="utf-8")
    assert "plcassistant_dashboard_version: 11" in text
    assert "0.1.23" in text or "unit-op" in text.lower() or "skid plant" in text.lower()
    assert "number.plcassistant_lt_tank_in" in text
...
def test_run_sh_refreshes_stock_missing_status_not_custom() -> None:
    ...
    # Explicit old versions only — do not refresh merely missing version 11.
    assert "plcassistant_dashboard_version:[[:space:]]*([1-9]|10)" in text
    ...
    assert "! grep -q 'plcassistant_dashboard_version: 11'" not in text
