...
def test_app_version_locked_to_integration() -> None:
    ...
    assert 'version: "0.1.37"' in text
    docker = (ROOT / "plc_assistant" / "Dockerfile").read_text(encoding="utf-8")
    assert "BUILD_VERSION=0.1.37" in docker
...
def test_bundled_yaml_exists() -> None:
    ...
    assert "plcassistant_dashboard_version: 17" in text
...
def test_run_sh_refreshes_stock_missing_status_not_custom() -> None:
    ...
    # Explicit old versions only — refresh 1–16 stock boards to v17 (SWD-183).
    assert "plcassistant_dashboard_version:[[:space:]]*([1-9]|1[0-6])" in text
