...
| 2 | Define Soft-PLC program organization model (tasks → programs → instances) | define | SWD-179 | To Do | [SWD-182](https://marcusknielsen.atlassian.net/browse/SWD-182) |
...
## Not yet specified
- Exact industrial metaphor for Soft-PLC (IEC Configuration/Resource vs CODESYS Application vs Logix Task/Program naming)
- How many concurrent tasks/programs/datablocks are in the first shippable slice vs later
- How integration “datablocks” bind to Soft-PLC programs (1:1 vs many tags per program)
- Hot-apply / restart policy across multiple programs
- Depth of online force/write vs monitor-only for v1
- Whether Python FBD remains the only authoring language (LD/ST deferred?)
- Migration path for existing wedge cascade instances onto generic PID
- Concrete system-test harness shape (HA+App+MQTT end-to-end vs in-repo fixtures)
...
## Next
`/define SWD-182` — Define Soft-PLC program organization model: first unblocked buildable slice
