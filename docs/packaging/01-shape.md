...
**Ownership (SWD-145/146/144/143/166):** The thin integration owns the stand-alone process simulator (`custom_components/plcassistant/dynamics/`), including a unit-op catalog + AST expression sandbox that compile into collected `ModelSpec`s, plus a sidebar **Dynamics** block editor (App 0.1.25+). Soft-PLC treats plant PVs as MQTT **IN** and stays on `HeldProcess`. Live default remains the code **skid** preset; composed documents are selectable/editable via the Dynamics tab / Options flow / `set_dynamics_preset`.
