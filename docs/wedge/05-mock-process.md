...
**Live path (SWD-146/144/143):** the thin integration runs the skid plant simulator (code `skid` default) and can load composed unit-op model documents (`skid_composed`, …) selected via Options flow / `set_dynamics_preset`. Soft-PLC stays mock-unaware (`HeldProcess`). Offline wedge acceptance still uses `MockProcess`.
